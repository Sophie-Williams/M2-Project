...
bool CPythonNetworkStream::RecvPointChange()
{
	...
	if (PointChange.dwVID == pInstance->GetVirtualID())
	{
		...
		/*if (PointChange.Type == POINT_GOLD) {
			...
			auto DoChanges = [=](long long val, bool IsGrowing) {
				long long wait = 0;
				if (abs(PointChange.value - val) < 10000)
					wait = 1;
				for (; val <= PointChange.value && IsGrowing; val++) {
			...
			std::thread mythread(DoChanges, static_cast<long long>(CPythonPlayer::Instance().GetStatus(PointChange.Type)), CPythonPlayer::Instance().GetStatus(PointChange.Type) < PointChange.value);
			if (mythread.joinable())
				mythread.detach(); // do not use join()
			else
				rkPlayer.SetStatus(PointChange.Type, PointChange.value);
		}
		else if (PointChange.Type == POINT_GAYA) {
			auto DoChanges = [=](long long val, bool IsGrowing) {
				long long wait = 0;
				if (abs(PointChange.value - val) < 10000)
					wait = 1;
		...
		if (POINT_GAYA == PointChange.Type)
		{
			if (PointChange.amount > 0)
			{
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnPickGem", Py_BuildValue("(l)", PointChange.amount));
			}
		}
#endif	
...
#ifdef ENABLE_CHEQUE_SYSTEM
bool CPythonNetworkStream::SendExchangeChequeAddPacket(long cheque) //elk ???
{
	if (!__CanActMainInstance())
		return true;
