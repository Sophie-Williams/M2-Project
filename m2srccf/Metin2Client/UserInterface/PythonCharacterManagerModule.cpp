...
PyObject * chrmgrRegisterRaceType(PyObject* poSelf, PyObject* poArgs)
{
	char * szName;
	if (!PyTuple_GetString(poArgs, 0, &szName))
		return Py_BadArgument();
	...
	char * szFolder;
	if (!PyTuple_GetString(poArgs, 1, &szFolder))
		return Py_BadArgument();

	CRaceManager::Instance().SetFolderName(szName, szFolder);
...
void initchrmgr()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "RegisterRaceType",			chrmgrRegisterRaceType,					 METH_VARARGS },
	...
	//PyModule_AddIntConstant(poModule, "NAMECOLOR_NPC_PC", CInstanceBase::NAMECOLOR_NPC_PC);
	PyModule_AddIntConstant(poModule, "NAMECOLOR_METIN", CInstanceBase::NAMECOLOR_METIN);
	PyModule_AddIntConstant(poModule, "NAMECOLOR_MINERAL", CInstanceBase::NAMECOLOR_MINERAL);
