...
void CPythonShop::AddPrivateShopItemStock(TItemPos ItemPos, BYTE dwDisplayPos, long long dwPrice, long dwCheque)
#else
void CPythonShop::AddPrivateShopItemStock(TItemPos ItemPos, BYTE dwDisplayPos, long long dwPrice)
#endif
{
...
#ifdef ENABLE_CHEQUE_SYSTEM
long CPythonShop::GetPrivateShopItemCheque(TItemPos ItemPos)
{
	TPrivateShopItemStock::iterator itor = m_PrivateShopItemStock.find(ItemPos);
...
#ifdef ENABLE_CHEQUE_SYSTEM
	long iCheque;
	if (!PyTuple_GetLong(poArgs, 4, &iCheque))
		return Py_BuildException();
#endif
