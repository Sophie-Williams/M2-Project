...
#ifdef ENABLE_CHEQUE_SYSTEM
	//rkNetStream.SendItemDropPacket(TItemPos(RESERVED_WINDOW, 0), (DWORD)iElk, 0); //long long???
	rkNetStream.SendItemDropPacket(TItemPos(RESERVED_WINDOW, 0), (DWORD) iElk, 0); //long long???
#else
	//rkNetStream.SendItemDropPacket(TItemPos(RESERVED_WINDOW, 0), (DWORD) iElk); //long long ????
	rkNetStream.SendItemDropPacket(TItemPos(RESERVED_WINDOW, 0), (DWORD)iElk); //long long ????
#endif
...
#ifdef ENABLE_CHEQUE_SYSTEM
	rkNetStream.SendItemDropPacketNew(TItemPos(RESERVED_WINDOW, 0), (long long) iElk, 0, 0); //long long???
#else
	rkNetStream.SendItemDropPacketNew(TItemPos(RESERVED_WINDOW, 0), (long long) iElk, 0); //long long???
#endif
...
PyObject * netSendChequeDropPacketNew(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!PyTuple_GetLong(poArgs, 0, &Cheque))
		return Py_BuildException();

	CPythonNetworkStream & rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendItemDropPacketNew(TItemPos(RESERVED_WINDOW, 0), 0, 0, (long) Cheque);
