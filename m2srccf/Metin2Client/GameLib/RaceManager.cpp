...
/*
// Force to load NPC1 folder //
bool __IsNPC1(unsigned race)
{
	if (race >= 60000)
		return true;
	...
	if (race >= 30301 && race <= 30307)
		return true;

	return false;
}
*/
// Force to load NPC2 folder //

/*
bool __IsNPC2(unsigned race)
{
	if (race == 20095 || race == 20416 || race == 20382 || race == 20384 || race == 20385 || race == 20387 || race == 20388 || race == 20389 || race == 20390 || race == 20391 || race == 20397 || race == 20398 || race == 20399 || race == 20371 || race == 20375)
		return true;

	if (race >= 20357 && race <= 20366)
		return true;

	return false;
}*/

// Force to load NPC_MOUNT folder //

/*
bool __IsNPCMount(unsigned race)
{
	if (race >= 20233 && race <= 20242)
		return true;
...
// Force to load MONSTER1 folder //

/*

bool __IsMonster1(unsigned race)
{
	if (race == 20422)
		return true;
...
void __GetRaceResourcePathes(unsigned race, std::vector <std::string>& vec_stPathes)
{
	...
	else if (__IsNPCMount(race))
	{
		vec_stPathes.push_back("d:/ymir work/npc_mount/");
	}
	else if (__IsMonster1(race))
	{
		vec_stPathes.push_back("d:/ymir work/monster/");
	}
...
void CRaceManager::__GetRaceResourcePathes(string race, std::vector <std::string>& vec_stPathes)
{
	auto fRaceName = m_kMap_stRaceName_stFolderName.find(race);

	if (m_kMap_stRaceName_stFolderName.end() == fRaceName)
	{
		vec_stPathes.push_back("d:/ymir work/guild/");
		return;
	}

	const std::string& c_strFolderName = fRaceName->second;

	if (c_strFolderName == "MOUNT")
	{
		vec_stPathes.push_back("d:/ymir work/mount/");
	}
	else if (c_strFolderName == "PET")
	{
		vec_stPathes.push_back("d:/ymir work/pet/");
	}
	else if (c_strFolderName == "MONSTER")
	{
		vec_stPathes.push_back("d:/ymir work/monster/");
	}
	else if (c_strFolderName == "NPC")
	{
		vec_stPathes.push_back("d:/ymir work/npc/");
	}
	else
...
CRaceData* CRaceManager::__LoadRaceData(DWORD dwRaceIndex)
{
	...
	__GetRaceResourcePathes(c_rstRaceName, vec_stFullPathName);
...
void CRaceManager::SetFolderName(const char * c_szName, const char * c_szFolderName)
{
	m_kMap_stRaceName_stFolderName.insert(std::map<std::string, std::string>::value_type(c_szName, c_szFolderName));
}
