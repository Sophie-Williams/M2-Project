...
class CRaceManager : public CSingleton<CRaceManager>
{
	...
	public:
		...
		void SetFolderName(const char* c_szName, const char * c_szFolderName);
		void __GetRaceResourcePathes(string race, std::vector <std::string>& vec_stPathes);
	...
	protected:
		...
		std::map<string, string>            m_kMap_stRaceName_stFolderName;
