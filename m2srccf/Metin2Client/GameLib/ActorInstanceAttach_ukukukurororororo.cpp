...
#define ENABLE_AUTODETECT_ASSASSIN_RODNPICK_BONE
void CActorInstance::AttachWeapon(DWORD dwParentPartIndex, DWORD dwPartIndex, CItemData * pItemData)
{
	if (!pItemData)
		return;
	
#ifdef ENABLE_AUTODETECT_ASSASSIN_RODNPICK_BONE
	const char * szBoneName;
	if ((GetRace()==5) && (pItemData->GetType()==CItemData::ITEM_TYPE_ROD || pItemData->GetType()==CItemData::ITEM_TYPE_PICK))
	{
