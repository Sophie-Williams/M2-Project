...
class CLogFile : public CSingleton<CLogFile>
{
	public:
		...
		void Initialize()
		{
			...
			strftime(buffer, sizeof(buffer), "./log/%d_%m_%Y___%H_%M_%S.sys_log", timeinfo);
			const char* str(buffer);

			//m_fp = fopen("./log/sys_log.metin2", "w");
			m_fp = fopen(str, "w");
		}
...
void TraceError(const char* c_szFormat, ...)
{
	...
	char szBuf[DEBUG_STRING_MAX_LEN+10];

	strncpy(szBuf, "M2 SYSERR :: ", DEBUG_STRING_MAX_LEN);
...
void OpenLogFile(bool bUseLogFIle)
{
	...
	time(&rawtime);
	timeinfo = localtime(&rawtime);
	strftime(buffer, sizeof(buffer), "./log/%d_%m_%Y___%H_%M_%S.sys_err", timeinfo);
	const char* str(buffer);
