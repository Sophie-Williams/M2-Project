...
bool CGrannyMaterial::IsIn(const char* c_szImageName, int* piStage)
{
	...
	strImageName = CFileNameHelper::NoPath(strImageName);
	...
	if (pgrnDiffuseTexture)
	{
		...
		strDiffuseFileName = CFileNameHelper::NoPath(strDiffuseFileName);
	...
	if (pgrnOpacityTexture)
	{
		...
		strOpacityFileName = CFileNameHelper::NoPath(strOpacityFileName);
...
CGraphicImage* CGrannyMaterial::__GetImagePointer(const char* fileName)
{
	...
	if (CFileNameHelper::GetPath(localpath) == "")
	{
		char localFileName[256];
		const std::string& modelLocalPath = GetModelLocalPath();
	...
	std::string TextureName = CFileNameHelper::NoPath(localpath);
	const std::string& modelLocalPath = GetModelLocalPath();
	std::string TexturePath = modelLocalPath + TextureName;
	...
	//TraceError("Grannymaterial: Loading normal texture %s", localFileName);
	//CResource* pResource = rkResMgr.GetResourcePointer(fileName);
	//return static_cast<CGraphicImage*>(pResource);
}
