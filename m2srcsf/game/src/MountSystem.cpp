...
bool CMountActor::Mount(LPITEM mountItem)
{
	...
	m_pkOwner->AddAffect(AFFECT_MOUNT, POINT_MOUNT, m_dwVnum, AFF_NONE, (uint32_t)mountItem->GetSocket(0) - time(0), 0, true);
	...
	for (int32_t i = 0; i < ITEM_APPLY_MAX_NUM; ++i)
	{
		...
		m_pkOwner->AddAffect(AFFECT_MOUNT_BONUS, aApplyInfo[mountItem->GetProto()->aApplies[i].bType].bPointType, mountItem->GetProto()->aApplies[i].lValue, AFF_NONE, (uint32_t)mountItem->GetSocket(0) - time(0), 0, false);
	}
...
uint32_t CMountSystem::CountSummoned() const
{
	uint32_t count = 0;
