...
EVENTFUNC(unbinding_expire_event)
{
	...
	int32_t iNextExpire = pItem->GetBind() - processing_time / passes_per_sec;
	if (iNextExpire <= 2)
	{
		pItem->Bind(0);
...
{
	uint32_t dwImmuneFlag = m_pProto->dwImmuneFlag;
	for (int32_t i = 0; i < ITEM_ATTRIBUTE_MAX_NUM; ++i)
	{
		if (GetAttributeType(i))
		{
			...
			if (ia.bType == APPLY_IMMUNE_FALL && !IS_SET(dwImmuneFlag, IMMUNE_FALL))
				SET_BIT(dwImmuneFlag, IMMUNE_FALL);
			if (ia.bType == APPLY_IMMUNE_SLOW && !IS_SET(dwImmuneFlag, IMMUNE_SLOW))
