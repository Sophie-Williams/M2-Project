	...
	void operator () (LPENTITY ent)
	{
		...
		if (!pkChrVictim->Damage(m_pkChr, iDam, dt) && !pkChrVictim->IsStun())
		{
			...
			if (IS_SET(m_pkSk->dwFlag, SKILL_FLAG_KNOCKBACK))
			{
				...
				sys_log(0, "KNOCKBACK! %s -> %s (%d %d) -> (%d %d)", m_pkChr->GetName(), pkChrVictim->GetName(), pkChrVictim->GetX(), pkChrVictim->GetY(), (int32_t)(pkChrVictim->GetX() + fx), (int32_t)(pkChrVictim->GetY() + fy));
...
#ifdef ENABLE_NEW_GYEONGGONG_SKILL
int32_t CHARACTER::ComputeGyeongGongSkill(uint32_t dwVnum, LPCHARACTER pkVictim, uint8_t bSkillLevel)
{
	if (IsPolymorphed())
		return BATTLE_NONE;
	...
	SetPolyVarForAttack(this, pkSk, pkWeapon);
	int32_t iAmount = (int32_t) pkSk->kPointPoly2.Eval();
