...
namespace quest
{
	...
	class CQuestManager : public singleton<CQuestManager>
	{
		...
		public:
			...
			void		DPlayer(uint32_t pc);
			void		DMonster(uint32_t pc);

			void		Death(uint32_t pc);
			void		Dies(uint32_t mob, uint32_t pc);
			void		Respawn(uint32_t pc);

			
			void		Destroy(uint32_t pc);
			
			void		FishingSuccess(uint32_t pc);
			void		FishingFailed(uint32_t pc);
			
			
			void		MiningSuccess(uint32_t pc);
			void		MiningFailed(uint32_t pc);
			
			void		RefineSuccess(uint32_t pc);
			void		RefineFailed(uint32_t pc);
			...
			void QuestButtonByName(const LPCHARACTER& ch, const std::string& qname);
			void PrintQuestIndexList();
#endif
