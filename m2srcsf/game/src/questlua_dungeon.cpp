...
namespace quest
{
	...
	int32_t dungeon_set_damage_from_race(lua_State* L)
	{
		...
		LPCHARACTER ch = CHARACTER_MANAGER::instance().Find(vid);
		if (ch)
			ch->SetNoDamageRaceFlag((uint32_t) lua_tonumber(L, 1));
		return 0;
	}
	...
	int32_t dungeon_set_damage_from_affect(lua_State* L)
	{
		...
		LPCHARACTER ch = CHARACTER_MANAGER::instance().Find(vid);
		if (ch)
			ch->SetNoDamageAffectFlag((uint32_t) lua_tonumber(L, 1));
		return 0;
	}
	...
	int32_t dungeon_unset_damage_from_race(lua_State* L)
	{
		...
		LPCHARACTER ch = CHARACTER_MANAGER::instance().Find(vid);
		if (ch)
			ch->UnsetNoDamageRaceFlag((uint32_t) lua_tonumber(L, 1));
		return 0;
	}
	...
	int32_t dungeon_unset_damage_from_affect(lua_State* L)
	{
		...
		LPCHARACTER ch = CHARACTER_MANAGER::instance().Find(vid);
		if (ch)
			ch->UnsetNoDamageAffectFlag((uint32_t) lua_tonumber(L, 1));
		return 0;
	}
