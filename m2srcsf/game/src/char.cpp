...
void CHARACTER::OpenMyShop(const char * c_pszSign, TShopItemTable * pTable, uint8_t bItemCount)
{
...
#ifdef ENABLE_CHEQUE_SYSTEM
		nTotalCheque += static_cast<int32_t>((pTable + n)->cheque);
#endif
	}
...
#ifdef ENABLE_CHEQUE_SYSTEM
	nTotalCheque += static_cast<int32_t>(GetCheque());
#endif
	...
	if (CHEQUE_MAX <= nTotalCheque)
	{
		...
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You can't create a shop with more than %ld won."), CHEQUE_MAX-1);
		return;
	}
#endif
...
void CHARACTER::RestartAtSamePos()
{
	...
	quest::CQuestManager::instance().Respawn(GetPlayerID());
...
bool CHARACTER::Show(int32_t lMapIndex, int32_t x, int32_t y, int32_t z, bool bShowSpawnMotion, bool bAggressive)
{
...
#ifdef ENABLE_CHEQUE_SYSTEM
	int32_t iCheque = GetPoint(POINT_CHEQUE);
#endif
...
void CHARACTER::PointChange(uint8_t type, int64_t amount, bool bAmount, bool bBroadcast)
{
	...
	switch (type)
	{
		...
		case POINT_GOLD:
			...
			case POINT_GAYA:
			{
				const int32_t nTotalGaya = static_cast<int32_t>(GetGaya()) + static_cast<int32_t>(amount);

				if (GAYA_MAX <= nTotalGaya)
				{
					sys_err("[OVERFLOW_GAYA] OriGaya %ld AddedGaya %lld id %u Name %s ", GetGaya(), amount, GetPlayerID(), GetName());
		...
		case POINT_CHEQUE:
			{
				const int32_t nTotalCheque = static_cast<int32_t>(GetCheque()) + static_cast<int64_t>(amount);
				if (CHEQUE_MAX <= nTotalCheque)
				{
					sys_err("[OVERFLOW_CHEQUE] OriCheque %ld AddedCheque %lld id %u Name %s ", GetCheque(), amount, GetPlayerID(), GetName());
...
void CHARACTER::SetParty(LPPARTY pkParty)
{
	...
	if (m_pkDungeon && IsPC())
	{			
		sys_err("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");
		SetDungeon(NULL);
	}
...
void CHARACTER::ResetPoint(int32_t iLv)
{
	...
	PointChange(POINT_STAT, ((MINMAX(1, iLv, 119) - 1) * 3) + GetPoint(POINT_LEVEL_STEP) - GetPoint(POINT_STAT));
...
{
	const int32_t METIN_STONE_NUM = 16;
...
void CHARACTER::ChannelSwitch(int32_t new_ch){
	int32_t lAddr;
	int32_t lMapIndex;
	uint16_t wPort;
	int32_t x = this->GetX();
	...
	std::map<uint16_t, int32_t>micha;
	
	for(int32_t i = 0; i < 3; i++){ 
		for(int32_t i2 = 0; i2 < 3; i2++){ 
			micha[13*1000 + i*10 + i2] = i+1;
		}        
	}
...
{
	std::vector<int32_t> needkeys;
	for (int32_t i = INVENTORY_NEED_KEY_START; i <= (INVENTORY_LOCKED_PAGE_COUNT*INVENTORY_NEED_KEY_INCREASE)+1; i++)
		for (int32_t j = 0; j < 3; j++)
			needkeys.push_back(i);
	int32_t needkey = needkeys[Inven_Point()];
	int32_t keycount = CountSpecifyItem(INVENTORY_OPEN_KEY_VNUM) + CountSpecifyItem(INVENTORY_OPEN_KEY_VNUM2);
	if (keycount >= needkey) {
		int32_t willdelete = INVENTORY_START_DELETE_VNUM;
		while (needkey) {
...
#define MOUNT_SYSTEM_FIX_POLY
	...
	if (GetMapIndex() == 252 || GetMapIndex() >= 2520000 && GetMapIndex() <= 2529999)
	{
		return;
	}
...
void CHARACTER::SortInven(uint8_t option)
{
	if (IsDead())
		return;
	...
	LPITEM myitems;
	const auto size = static_cast<uint16_t>(INVENTORY_MAX_NUM);
	...
	for (const auto& getitem : all) {
		...
		if (!table)
			continue;
		static const std::initializer_list<uint32_t> out = { ITEM_AUTO_HP_RECOVERY_S, ITEM_AUTO_HP_RECOVERY_M, ITEM_AUTO_HP_RECOVERY_L, ITEM_AUTO_HP_RECOVERY_X, ITEM_AUTO_SP_RECOVERY_S, ITEM_AUTO_SP_RECOVERY_M, ITEM_AUTO_SP_RECOVERY_L, ITEM_AUTO_SP_RECOVERY_X };
