...
CClientPackageCryptInfo::CClientPackageCryptInfo() : m_nCryptKeyPackageCnt(0), m_pSerializedCryptKeyStream(0){}
...
bool CClientPackageCryptInfo::LoadPackageCryptInfo( const char* pCryptInfoDir )
{
	...
	while ((pDirEnt = readdir(pDir)))
	{
			...
			{
				sys_log(0, "[PackageCryptInfo] %s is crypt file. continue !", pDirEnt->d_name);
