...
int64_t CShop::Buy(LPCHARACTER ch, uint8_t pos)
{
...
#ifdef ENABLE_SHOP_USE_CHEQUE
	uint32_t dwCheque = r_item.cheque;
#endif
...
#ifdef ENABLE_SHOP_USE_CHEQUE
	if ((int64_t)dwPrice>0 && (int32_t)dwCheque > 0)
	{
		if (ch->GetGold() < (int64_t)dwPrice || ch->GetCheque() < (int32_t)dwCheque)
	...
	{
		if (ch->GetCheque() < (int32_t)dwCheque)
			return SHOP_SUBHEADER_GC_NOT_ENOUGH_CHEQUE;
	}
#else
