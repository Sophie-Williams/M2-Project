...
void CGuild::SendAllGradePacket(LPCHARACTER ch)
{
	...
	for (uint8_t i = 0; i < GUILD_GRADE_COUNT; i++)
	{
		uint8_t j = i+1;
		buf.write(&j, 1);
...
void CGuild::RequestDepositMoney(LPCHARACTER ch, int64_t iGold)
{
...
void CGuild::RequestWithdrawMoney(LPCHARACTER ch, int64_t iGold)
{
	if (false==ch->CanDeposit())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<길드> 잠시후에 이용해주십시오"));
...
void CGuild::RecvMoneyChange(int64_t iGold)
{
	m_data.gold = iGold;
...
void CGuild::RecvWithdrawMoneyGive(int64_t iChangeGold)
{
