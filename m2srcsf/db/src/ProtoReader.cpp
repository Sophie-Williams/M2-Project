...
int32_t get_Item_WearFlag_Value(string inputString) 
{
	...
	string arWearrFlag[] = {"WEAR_BODY", "WEAR_HEAD", "WEAR_FOOTS", "WEAR_WRIST", "WEAR_WEAPON", "WEAR_NECK", "WEAR_EAR", "WEAR_SHIELD", "WEAR_UNIQUE",
					"WEAR_ARROW", "WEAR_HAIR", "WEAR_ABILITY", "WEAR_COSTUME_BODY", "WEAR_COSTUME_HAIR", "WEAR_COSTUME_WEAPON", 
								#ifdef __ACCE_SYSTEM__
								"WEAR_COSTUME_ACCE", 
								#endif
								"WEAR_COSTUME_MOUNT"
...
#endif
		,"APPLY_RESIST_WOLFMAN", "APPLY_ATTBONUS_WOLFMAN", "APPLY_RESIST_CLAW"
	};
...
bool Set_Proto_Mob_Table(TMobTable *mobTable, cCsvTable &csvTable,std::map<int32_t,const char*> &nameMap)
{
	...
	str_to_number(mobTable->bSkillRange, csvTable.AsStringByIndex(col++));

	sys_log(0, "[ M2P ]::[ LOAD ]::[ MOB ]::[ %d ]::[ %s ]", mobTable->dwVnum, mobTable->szLocaleName);
...
bool Set_Proto_Item_Table(TItemTable *itemTable, cCsvTable &csvTable,std::map<int32_t,const char*> &nameMap)
{
	...
	sys_log(0, "[ M2P ]::[ LOAD ]::[ ITEM ]::[ %d ]::[ %s ]", itemTable->dwVnum, itemTable->szLocaleName);
