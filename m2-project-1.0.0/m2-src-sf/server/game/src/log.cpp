...
void LogManager::LevelLog(LPCHARACTER pChar, int64_t level, uint32_t playhour)
{
	if (true == LC_IsEurope())
	{
		uint32_t aid = 0;
...
void LogManager::Ore(int32_t iPid, int32_t iOre, int32_t iVnum, int32_t iCount, int32_t iMap, int32_t iPosX, int32_t iPosY, int32_t iPosZ)
{
	Query(
		"INSERT DELAYED INTO log_ore%s ("
			"`pid`, `ore`, `vnum`, `count`, `map_index`, `map_x`, `map_y`, `map_z`, `date`"
		") VALUES ("
			"'%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', now()"
		")",

		get_table_postfix(), iPid, iOre, iVnum, iCount, iMap, iPosX, iPosY, iPosZ
	);
}

void LogManager::Fish(int32_t iPid, int32_t iVnum, int32_t iMap, int32_t iPosX, int32_t iPosY, int32_t iPosZ)
{
	Query(
		"INSERT DELAYED INTO log_fish%s ("
			"`pid`, `vnum`, `map_index`, `map_x`, `map_y`, `map_z`, `date`"
		") VALUES ("
			"'%d', '%d', '%d', '%d', '%d', '%d', now()"
		")",

		get_table_postfix(), iPid, iVnum, iMap, iPosX, iPosY, iPosZ
	);
}

void LogManager::Exchange(int32_t iPidFrom, int32_t iPidTo, int32_t iVnum, int32_t iCountBeforeFrom, int32_t iCountAfterFrom, int32_t iCountBeforeTo, int32_t iCountAfterTo, int32_t iCount, int32_t iMap, int32_t iPosX, int32_t iPosY, int32_t iPosZ)
{
	Query(
		"INSERT DELAYED INTO log_exchange%s ("
			"`pid_from`, `pid_to`, `vnum`, `from_count_before`, `from_count_after`, `to_count_before`, `to_count_after`, `count`, `map_index`, `map_x`, `map_y`, `map_z`, `date`"
		") VALUES ("
			"'%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', now()"
		")",

		get_table_postfix(), iPidFrom, iPidTo, iVnum, iCountBeforeFrom, iCountAfterFrom, iCountBeforeTo, iCountAfterTo, iCount, iMap, iPosX, iPosY, iPosZ
	);
}

void LogManager::ExchangeGold(int32_t iPidFrom, int32_t iPidTo, int64_t iCountBeforeFrom, int64_t iCountAfterFrom, int64_t iCountBeforeTo, int64_t iCountAfterTo, int64_t iCount, int32_t iMap, int32_t iPosX, int32_t iPosY, int32_t iPosZ)
{
	Query(
		"INSERT DELAYED INTO log_exchange_gold%s ("
			"`pid_from`, `pid_to`, `from_count_before`, `from_count_after`, `to_count_before`, `to_count_after`, `gold`, `map_index`, `map_x`, `map_y`, `map_z`, `date`"
		") VALUES ("
			"'%d', '%d', '%lld', '%lld', '%lld', '%lld', '%lld', '%d', '%d', '%d', '%d', now()"
		")",

		get_table_postfix(), iPidFrom, iPidTo, iCountBeforeFrom, iCountAfterFrom, iCountBeforeTo, iCountAfterTo, iCount, iMap, iPosX, iPosY, iPosZ
	);
}

void LogManager::ExchangeCheque(int32_t iPidFrom, int32_t iPidTo, int32_t iCountBeforeFrom, int32_t iCountAfterFrom, int32_t iCountBeforeTo, int32_t iCountAfterTo, int32_t iCount, int32_t iMap, int32_t iPosX, int32_t iPosY, int32_t iPosZ)
{
	Query(
		"INSERT DELAYED INTO log_exchange_cheque%s ("
			"`pid_from`, `pid_to`, `from_count_before`, `from_count_after`, `to_count_before`, `to_count_after`, `cheque`, `map_index`, `map_x`, `map_y`, `map_z`, `date`"
		") VALUES ("
			"'%d', '%d', '%ld', '%ld', '%ld', '%ld', '%ld', '%d', '%d', '%d', '%d', now()"
		")",
...
void LogManager::PickupGold(int32_t iPid, int64_t iGoldBefore, int64_t iGoldAfter, int64_t iGold, int32_t iMap, int32_t iPosX, int32_t iPosY, int32_t iPosZ)
{
	Query(
		"INSERT DELAYED INTO log_pickup_gold%s ("
			"`pid`, `gold_before`, `gold_after`, `gold`, `map_index`, `map_x`, `map_y`, `map_z`, `date`"
		") VALUES ("
			"'%d', '%lld', '%lld', '%d', '%d', '%d', '%d', '%d', now()"
		")",

		get_table_postfix(), iPid, iGoldBefore, iGoldAfter, iGold, iMap, iPosX, iPosY, iPosZ
	);
}

void LogManager::Pickup(int32_t iPid, int32_t iVnum, int32_t iCountBefore, int32_t iCountAfter, int32_t iCount, int32_t iMap, int32_t iPosX, int32_t iPosY, int32_t iPosZ)
{
	Query(
		"INSERT DELAYED INTO log_pickup%s ("
			"`pid`, `vnum`, `count_before`, `count_after`, `count`, `map_index`, `map_x`, `map_y`, `map_z`, `date`"
		") VALUES ("
			"'%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', now()"
		")",

		get_table_postfix(), iPid, iVnum, iCountBefore, iCountAfter, iCount, iMap, iPosX, iPosY, iPosZ
	);
}

void LogManager::Refine(int32_t iPid, int32_t iOldVnum, int32_t iNewVnum, int32_t iMap, int32_t iPosX, int32_t iPosY, int32_t iPosZ)
{
	Query(
		"INSERT DELAYED INTO log_refine%s ("
			"`pid`, `old_vnum`, `new_vnum`, `map_index`, `map_x`, `map_y`, `map_z`, `date`"
		") VALUES ("
			"'%d', '%d', '%d', '%d', '%d', '%d', '%d', now()"
		")",

		get_table_postfix(), iPid, iOldVnum, iNewVnum, iMap, iPosX, iPosY, iPosZ
	);
}

void LogManager::Destroy(int32_t iPid, int32_t iVnum, int32_t iCountBefore, int32_t iCountAfter, int32_t iCount, int32_t iMap, int32_t iPosX, int32_t iPosY, int32_t iPosZ)
{
	Query(
		"INSERT DELAYED INTO log_destroy%s ("
			"`pid`, `vnum`, `count_before`, `count_after`, `count`, `map_index`, `map_x`, `map_y`, `map_z`, `date`"
		") VALUES ("
			"'%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', now()"
		")",
...
void LogManager::Drop(int32_t iPid, int32_t iVnum, int32_t iCountBefore, int32_t iCountAfter, int32_t iCount, int32_t iMap, int32_t iPosX, int32_t iPosY, int32_t iPosZ)
{
	Query(
		"INSERT DELAYED INTO log_drop%s ("
			"`pid`, `vnum`, `count_before`, `count_after`, `count`, `map_index`, `map_x`, `map_y`, `map_z`, `date`"
		") VALUES ("
			"'%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', now()"
		")",
...
void LogManager::DropGold(int32_t iPid, int64_t iGoldBefore, int64_t iGoldAfter, int64_t iGold, int32_t iMap, int32_t iPosX, int32_t iPosY, int32_t iPosZ)
{
	Query(
		"INSERT DELAYED INTO log_drop_gold%s ("
			"`pid`, `gold_before`, `gold_after`, `gold`, `map_index`, `map_x`, `map_y`, `map_z`, `date`"
		") VALUES ("
			"'%d', '%lld', '%lld', '%d', '%d', '%d', '%d', '%d', now()"
		")",
...
void LogManager::DropCheque(int32_t iPid, int32_t iChequeBefore, int32_t iChequeAfter, int32_t iCheque, int32_t iMap, int32_t iPosX, int32_t iPosY, int32_t iPosZ)
{
	Query(
		"INSERT DELAYED INTO log_drop_cheque%s ("
			"`pid`, `cheque_before`, `cheque_after`, `cheque`, `map_index`, `map_x`, `map_y`, `map_z`, `date`"
		") VALUES ("
			"'%d', '%ld', '%ld', '%d', '%d', '%d', '%d', '%d', now()"
		")",

		get_table_postfix(), iPid, iChequeBefore, iChequeAfter, iCheque, iMap, iPosX, iPosY, iPosZ
	);
}

void LogManager::PickupCheque(int32_t iPid, int32_t iChequeBefore, int32_t iChequeAfter, int32_t iCheque, int32_t iMap, int32_t iPosX, int32_t iPosY, int32_t iPosZ)
{
	Query(
		"INSERT DELAYED INTO log_pickup_cheque%s ("
			"`pid`, `cheque_before`, `cheque_after`, `cheque`, `map_index`, `map_x`, `map_y`, `map_z`, `date`"
		") VALUES ("
			"'%d', '%ld', '%ld', '%d', '%d', '%d', '%d', '%d', now()"
		")",
