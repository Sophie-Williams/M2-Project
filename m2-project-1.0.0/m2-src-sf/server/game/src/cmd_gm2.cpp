...
ACMD(do_mob_map)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Syntax: mm <vnum> <number (0 <> 100)>");
		return;
	}
		...
		case 9: 
			{
				...
				long before_gaya = tch->GetGaya();
				tch->PointChange(POINT_GAYA, gaya, true);
				long after_gaya = tch->GetGaya();
		...
		case 10: 
			{
				...
				tch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Cheque: ADD[%ld] TOTAL[%ld]"), cheque, tch->GetCheque());
			}
			break;
#endif
	}
	...
	if (*arg1)
	{
		switch (LOWER(*arg1))
		{
			...
			case 'q':
				...
				ch->ChatPacket(CHAT_TYPE_INFO, "The /reload q command is disabled !");

				char buf[64];
				snprintf(buf, sizeof(buf), "Please, don't use the /reload q command !");
				SendNotice(buf);
				...
				break;
			case 'f':
				ch->ChatPacket(CHAT_TYPE_INFO, "Reloading Fishing information.");
				fishing::Initialize();
				break;
			case 'a':
		...
		case 'c':
			{
				...
				if (test_server || GMLevel == GM_PLAYER)
				{
					...
					if (ch->GetGold() < -static_cast<long long>(t->dwPrice))
					{
