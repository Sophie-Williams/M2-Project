...
EVENTFUNC(ready_to_start_event)
{
	...
	if (chA == NULL || chB == NULL)
	{
		...
		if (chA != NULL)
		{
			...
			sys_log(0, "ARENA: 11 Oppernent is disappered. MyPID(%d) OppPID(%d)", pArena->GetPlayerAPID(), pArena->GetPlayerBPID());
		}

		if (chB != NULL)
		{
			chB->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("대련 상대가 사라져 대련을 종료합니다."));
			sys_log(0, "ARENA: 22 Oppernent is disappered. MyPID(%d) OppPID(%d)", pArena->GetPlayerBPID(), pArena->GetPlayerAPID());
		}
