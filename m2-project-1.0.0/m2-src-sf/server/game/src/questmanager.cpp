...
namespace quest
{
	...
	bool CQuestManager::Initialize()
	{
		...
		m_mapEventName.insert(TEventNameMap::value_type("death_by_monster", QUEST_D_MONSTER_EVENT));
		m_mapEventName.insert(TEventNameMap::value_type("death_by_player", QUEST_D_PLAYER_EVENT));

		m_mapEventName.insert(TEventNameMap::value_type("death", QUEST_DEATH_EVENT));
		m_mapEventName.insert(TEventNameMap::value_type("dies", QUEST_DIES_EVENT));
		m_mapEventName.insert(TEventNameMap::value_type("respawn", QUEST_RESPAWN_EVENT));
		
		m_mapEventName.insert(TEventNameMap::value_type("destroy", QUEST_DESTROY_EVENT));
		
		
		m_mapEventName.insert(TEventNameMap::value_type("fishing_success", QUEST_FISHING_SUCCESS_EVENT));
		m_mapEventName.insert(TEventNameMap::value_type("fishing_failed", QUEST_FISHING_FAILED_EVENT));
		
		
		m_mapEventName.insert(TEventNameMap::value_type("mining_success", QUEST_MINING_SUCCESS_EVENT));
		m_mapEventName.insert(TEventNameMap::value_type("mining_failed", QUEST_MINING_FAILED_EVENT));
		
		m_mapEventName.insert(TEventNameMap::value_type("refine_success", QUEST_REFINE_SUCCESS_EVENT));
		m_mapEventName.insert(TEventNameMap::value_type("refine_failed", QUEST_REFINE_FAILED_EVENT));
	...
	void CQuestManager::Death(uint32_t pc)
	{
		PC * pPC;
		...
		if ((pPC = GetPC(pc)))
		{
			...
			m_mapNPC[QUEST_NO_NPC].OnDeath(*pPC);
		}
		else
		{
			sys_err("QUEST no such pc id : %d", pc);
	...
	void CQuestManager::DPlayer(uint32_t pc)
	{
		PC * pPC;
		...
		if ((pPC = GetPC(pc)))
		{
			...
			m_mapNPC[QUEST_NO_NPC].OnDPlayer(*pPC);
		}
		
	}

	void CQuestManager::DMonster(uint32_t pc)
	{
		PC * pPC;
		...
		if ((pPC = GetPC(pc)))
		{
			...
			m_mapNPC[QUEST_NO_NPC].OnDMonster(*pPC);
		}
		else
		{
			sys_err("QUEST no such pc id : %d", pc);
	...
	void CQuestManager::Respawn(uint32_t pc)
	{
		PC * pPC;
	...
	void CQuestManager::Destroy(uint32_t pc)
	{
		PC * pPC;
		...
		if ((pPC = GetPC(pc)))
		{
			...
			m_mapNPC[QUEST_NO_NPC].OnDestroy(*pPC);
		}
		else
			sys_err("QUEST no such pc id : %d", pc);
	}

	void CQuestManager::FishingSuccess(uint32_t pc)
	{
		PC * pPC;
		...
		if ((pPC = GetPC(pc)))
		{
			...
			m_mapNPC[QUEST_NO_NPC].OnFishingSuccess(*pPC);
		}
		else
			sys_err("QUEST no such pc id : %d", pc);
	}

	void CQuestManager::FishingFailed(uint32_t pc)
	{
		PC * pPC;
		...
		if ((pPC = GetPC(pc)))
		{
			...
			m_mapNPC[QUEST_NO_NPC].OnFishingFailed(*pPC);
		}
		else
			sys_err("QUEST no such pc id : %d", pc);
	}

	void CQuestManager::MiningSuccess(uint32_t pc)
	{
		PC * pPC;
		...
		if ((pPC = GetPC(pc)))
		{
			...
			m_mapNPC[QUEST_NO_NPC].OnMiningSuccess(*pPC);
		}
		else
			sys_err("QUEST no such pc id : %d", pc);
	}

	void CQuestManager::MiningFailed(uint32_t pc)
	{
		PC * pPC;
		...
		if ((pPC = GetPC(pc)))
		{
			...
			m_mapNPC[QUEST_NO_NPC].OnMiningFailed(*pPC);
		}
		else
			sys_err("QUEST no such pc id : %d", pc);
	}

	void CQuestManager::RefineSuccess(uint32_t pc)
	{
		PC * pPC;
		...
		if ((pPC = GetPC(pc)))
		{
			...
			m_mapNPC[QUEST_NO_NPC].OnRefineSuccess(*pPC);
		}
		else
			sys_err("QUEST no such pc id : %d", pc);
	}

	void CQuestManager::RefineFailed(uint32_t pc)
	{
		PC * pPC;
		...
		if ((pPC = GetPC(pc)))
		{
			...
			m_mapNPC[QUEST_NO_NPC].OnRefineFailed(*pPC);
		}
		else
			sys_err("QUEST no such pc id : %d", pc);
	}
	...
	void CQuestManager::Dies(uint32_t mob, uint32_t pc)
	{
		PC * pPC;
		sys_log(0, "CQuestManager::MDEAD QUEST_MDEAD_EVENT (pc=%d, monsterid=%d)", pc, mob);
		...
		if ((pPC = GetPC(pc)))
		{
			...
			m_mapNPC[mob].OnDies(*pPC);
		}
		else
			sys_err("QUEST: no such (pc=%d, monsterid=%d)", pc, mob);
	}
	...
	bool CQuestManager::ServerTimer(uint32_t npc, uint32_t arg)
	{
		...
		sys_log(0, "M2::ServerTimer (%d) (%d) (%p)", npc, arg, GetPCForce(0));
	...
	void CQuestManager::PrintQuestIndexList() 
	{
	...
	{
		sys_log(0, "M2::AddServerTimer (%s) (%d) (%p)", name.c_str(), arg, get_pointer(event));
