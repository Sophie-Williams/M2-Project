...
const char* txt_gaya[] = {
	"MSG-6258857429148097",
	"MSG-5747075352699996",
	"MSG-5469589765867958-%d-%s",
	"MSG-3944631518304642",
	"MSG-1486646911355968-%d",
	"MSG-3630861837710655",
	"MSG-2617309755001827"
};
...
bool CHARACTER::CheckItemsFull()
{
	FILE 	*fp;
	char file_name[256+1];
	snprintf(file_name, sizeof(file_name), "%s/gaya/%d.txt", LocaleService_GetBasePath().c_str(), GetPlayerID());
...
void CHARACTER::InfoGayaMarker()
{
	...
	for (uint32_t i=0; i<info_items.size(); ++i){
...
bool CHARACTER::CheckSlotGayaMarket(int32_t slot)
{
...
void CHARACTER::BuyItemsGayaMarket(int32_t slot)
{
	if(GetGaya() >=  info_items[slot].value_2){
		...
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT(txt_gaya[0]));
		return;
	}
}
...
void CHARACTER::RefreshItemsGayaMarket()
{
	...
	for (int32_t i = 0; i<9; ++i){
		int32_t number = number(1, load_gaya_items.size()-1);
...
void CHARACTER::UpdateSlotGayaMarket(int32_t slot)
{
	FILE 	*fileID;
	char file_name[256+1];
	snprintf(file_name, sizeof(file_name), "%s/gaya/%d.txt", LocaleService_GetBasePath().c_str(), GetPlayerID());
...
void CHARACTER::CraftGayaItems(int32_t slot)
{
		LPITEM	item = GetItem(TItemPos(INVENTORY, slot));

		int32_t ID_Glimmerstone = load_gaya_values.glimmerstone;
		int32_t Count_Glimmerstone = load_gaya_values.glimmerstone_count;
		int32_t Grade_Stone = load_gaya_values.grade_stone;
		int32_t Point_Gaya = load_gaya_values.give_gaya;
		int32_t Random_Point_Gaya = number(1, 100);
		int32_t Prob_Gaya = load_gaya_values.prob_gaya;
		int32_t Cost_Gaya_Yang = load_gaya_values.cost_gaya_yang;
		...
		if (item->GetType() != ITEM_METIN || item->GetRefineLevel() > Grade_Stone){ 
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT(txt_gaya[1]));
			return;
		}

		if (CountSpecifyItem(ID_Glimmerstone) < Count_Glimmerstone){ 
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT(txt_gaya[2]), Count_Glimmerstone,item_glimmerstone->GetName());
			return;
		}

		if(GetGold() < Cost_Gaya_Yang){
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT(txt_gaya[3]));
			return;
		}
		...
		if (Random_Point_Gaya <= Prob_Gaya){
			...
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT(txt_gaya[4]), Point_Gaya);
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT(txt_gaya[5]));

		}else{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT(txt_gaya[6]));
		}
...
void CHARACTER::MarketGayaItems(int32_t slot)
{
	...
	int32_t ID_GayaMarketExpansion = load_gaya_values.gaya_expansion;
	...
	if (slot >= 3){
		if(CheckSlotGayaMarket(slot) == false){
			if(CountSpecifyItem(ID_GayaMarketExpansion) >= load_gaya_values.gaya_expansion_count){
				...
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT(txt_gaya[2]),load_gaya_values.gaya_expansion_count,item_gayarexpansion->GetName());
				return;
			}
		}else{
...
void CHARACTER::RefreshGayaItems()
{
	...
	int32_t ID_GayaMarketRefresh = load_gaya_values.gaya_refresh;
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT(txt_gaya[2]),load_gaya_values.gaya_refresh_count,item_gayarefresh->GetName());
		return;
	}
...
int32_t CHARACTER::GetGayaState(const std::string& state) const
{
...
void CHARACTER::SetGayaState(const std::string& state, int32_t szValue)
{
	quest::CQuestManager& q = quest::CQuestManager::instance();
	quest::PC* pPC = q.GetPC(GetPlayerID());
...
void CHARACTER::StartCheckTimeMarket()
{
	...
	GayaUpdateTime = event_create(check_time_market_event, info, PASSES_PER_SEC(5));	
}
