...
struct FuncPurge
{
	...
	void operator () (LPENTITY ent)
	{
		...
		if (!m_bAll && iDist >= 1000)	ÌÅÍ ÀÌ»ó¿¡ ÀÖ´Â °ÍµéÀº purge ÇÏÁö ¾Ê´Â´Ù.
			return;
...
ACMD(do_kill)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Utilisation /kill <nom du joueur>");
		return;
	}
...
ACMD(do_set)
{
	...
	if (!*arg1 || !*arg2 || !*arg3)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Utilisation: set <nom du joueur> <gold/exp/etc> <quantité>");
		return;
	}
		...
		case 9: 
			{
				...
				int32_t after_gaya = tch->GetGaya();
				if (0 == after_gaya && 0 != before_gaya)
				{
					LogManager::instance().CharLog(tch, gaya, "ZERO_GAYA", "GM");
				}
				tch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Gaya: Ajout de [%ld] Total[%ld]"), gaya, tch->GetGaya());
			}
			break;
#endif
		...
		case 10: 
			{
				...
				tch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Won: Ajout de [%ld] Total[%ld]"), cheque, tch->GetCheque());
			}
			break;
#endif
	}
	...
	if (*arg1)
	{
		switch (LOWER(*arg1))
		{
			...
			case 'p':
				...
				break;
			case 's':
				ch->ChatPacket(CHAT_TYPE_INFO, "Actualisation de notice string.");
			...
			case 'q':
				...
				char buf[64];
				snprintf(buf, sizeof(buf), "N'utilisez pas la commande reload q, elle casse tout !");
				SendNotice(buf);
...
{
	Ì ¾Æ´Ï°Å³ª block_chat_privilege°¡ ¾ø´Â »ç¶÷Àº ¸í·É¾î »ç¿ë ºÒ°¡
	...
	case JOB_SHAMAN:
		{
			...
			Í ÀÖ¾î¼­ ÀÓ½Ã·Î ¸ðµç Á÷±º ´Ù ÀÌ·± ¼Ó¼º µû¸§.
