...
void CClientManager::RESULT_LOGIN_BY_KEY(CPeer * peer, SQLMsg * msg)
{
	...
	{
		snprintf(szQuery, sizeof(szQuery), "SELECT `id`, `name`, `job`, `level`, `alignment`, `st`, `ht`, `dx`, `iq`, `part_main`, `part_hair`, "
		#ifdef __ACCE_SYSTEM__
		"`part_acce`, "
		#endif
		"`x`, `y`, `skill_group`, `change_name` FROM player%s WHERE `account_id` = %u", GetTablePostfix(), info->pAccountTable->id);
	}
	else
	{
		snprintf(szQuery, sizeof(szQuery), "SELECT `id`, `name`, `job`, `level`, `playtime`, `st`, `ht`, `dx`, `iq`, `part_main`, `part_hair`, "
...
void CClientManager::RESULT_LOGIN(CPeer * peer, SQLMsg * msg)
{
	...
	if (info->account_index == 0)
	{
		...
		else
		{
			...
			if (g_stLocale == "gb2312")
			{
				snprintf(queryStr, sizeof(queryStr), "SELECT `id`, `name`, `job`, `level`, `alignment`, `st`, `ht`, `dx`, `iq`, `part_main`, `part_hair`, "
			...
			{
				snprintf(queryStr, sizeof(queryStr), "SELECT `id`, `name`, `job`, `level`, `playtime`, `st`, `ht`, `dx`, `iq`, `part_main`, `part_hair`, "
