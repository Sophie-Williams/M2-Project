...
void CClientManager::QUERY_PLAYER_LOAD(CPeer * peer, uint32_t dwHandle, TPlayerLoadPacket * packet)
{
	...
	else
	{
        ...
        snprintf(queryStr, sizeof(queryStr),
                ...
                "UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(last_play), `horse_skill_point`, `gaya`,"

#ifdef ENABLE_CHEQUE_SYSTEM
												"`cheque`, "
#endif

				"`envanter` FROM player%s WHERE `id` = %d",
...
bool CreatePlayerTableFromRes(MYSQL_RES * res, TPlayerTable * pkTab)
{
	...
	str_to_number(pkTab->envanter, row[col++]);
	{
		...
		if (pkTab->level > 9)
		{
			...
			int32_t skill_point = MIN(20, pkTab->skills[121].bLevel) + MIN(20, pkTab->skills[124].bLevel) + MIN(10, pkTab->skills[131].bLevel) + MIN(20, pkTab->skills[141].bLevel) + MIN(20, pkTab->skills[142].bLevel);
