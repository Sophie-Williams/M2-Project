...
bool CClientManager::InitializeMobTable()
{
	...
	while (data.Next())
	{
		...
		if(isSameRow) {
			...
			mob_table->bMountCapacity = tempTable->bMountCapacity;
			...
			mob_table->bSkillRange = tempTable->bSkillRange;
		} else {
	...
	for (itertype(m_vec_mobTable) it = m_vec_mobTable.begin(); it != m_vec_mobTable.end(); it++)
	{
		...
		if (g_stLocaleNameColumn == "name")
		{
			snprintf(query, sizeof(query),
				...
				"("
				"`vnum`, `name`, `type`, `rank`, `battle_type`, `level`, `size`, `ai_flag`, `mount_capacity`, `setRaceFlag`, `setImmuneFlag`, "
				...
				"`sp_berserk`, `sp_stoneskin`, `sp_godspeed`, `sp_deathblow`, `sp_revive`, `skill_range`"
				") "
				"values ("

				"%d, \"%s\", %d, %d, %d, %d, %d, %u, %d, %u, %u, " 
				"%d, %d, %d, %d, '%s', "
				"%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, "
				"%d, %d, %d, %d, %d, %d, %d, %d, %d, "
				...
				t.dwVnum, t.szName, t.bType, t.bRank, t.bBattleType, t.bLevel, t.bSize, t.dwAIFlag, t.bMountCapacity, t.dwRaceFlag, t.dwImmuneFlag,
				...
				t.cResists[0], t.cResists[1], t.cResists[2], t.cResists[3], t.cResists[4], t.cResists[5], t.cResists[6],
				t.cResists[7], t.cResists[8], t.cResists[9], t.cResists[10], t.cResists[11], t.cResists[12], 
				...
				t.bBerserkPoint, t.bStoneSkinPoint, t.bGodSpeedPoint, t.bDeathBlowPoint, t.bRevivePoint, t.bSkillRange
				);
		}
		else
		{
			snprintf(query, sizeof(query),
				"replace into mob_proto%s"
				"("
				"`vnum`, `name`, %s, `type`, `rank`, `battle_type`, `level`, `size`, `ai_flag`, `mount_capacity`, `setRaceFlag`, `setImmuneFlag`, "
				"`on_click`, `empire`, `drop_item`, `resurrection_vnum`, `folder`, "
				"`st`, `dx`, `ht`, `iq`, `damage_min`, `damage_max`, `max_hp`, `regen_cycle`, `regen_percent`, `exp`, "
				"`gold_min`, `gold_max`, `def`, `attack_speed`, `move_speed`, `aggressive_hp_pct`, `aggressive_sight`, `attack_range`, `polymorph_item`, "
				...
				t.dwVnum, t.szName, t.szLocaleName, t.bType, t.bRank, t.bBattleType, t.bLevel, t.bSize, t.dwAIFlag, t.bMountCapacity, t.dwRaceFlag, t.dwImmuneFlag,
				t.bOnClickType, t.bEmpire, t.dwDropItemVnum, t.dwResurrectionVnum, t.szFolder,
				t.bStr, t.bDex, t.bCon, t.bInt, t.dwDamageRange[0], t.dwDamageRange[1], t.dwMaxHP, t.bRegenCycle, t.bRegenPercent, t.dwExp,
				t.dwGoldMin, t.dwGoldMax, t.wDef, t.sAttackSpeed, t.sMovingSpeed, t.bAggresiveHPPct, t.wAggressiveSight, t.wAttackRange, t.dwPolymorphItemVnum,
		...
		sys_log(0, "M2P::Load [ %d ]::[ %s ]", t.dwVnum, t.szLocaleName);
	}
	return true;
}
...
bool CClientManager::InitializeItemDropTable()
{
	...
	
	{
		...
		m_vecCommonDropItemTable.reserve((uint32_t) pResult->uiNumRows);
		...
		while ((row = mysql_fetch_row(pResult->pSQLResult)))
		{
			...
			const uint32_t scale = MAX(1000000/DROP_TABLE_SCALE,1);
	...
	
	{
		...
		m_vecDropItemTable.reserve((uint32_t) pResult->uiNumRows);
	...
	
	{
		...
		m_vecMobDropItemGroupKill.reserve((uint32_t) pResult->uiNumRows);
	...
	
	{
		...
		m_vecMobDropItemKill.reserve((uint32_t) pResult->uiNumRows);
	...
	
	{
		...
		m_vecMobDropItemGroupLevel.reserve((uint32_t) pResult->uiNumRows);
	...
	
	{
		...
		m_vecMobDropItemLevel.reserve((uint32_t) pResult->uiNumRows);
