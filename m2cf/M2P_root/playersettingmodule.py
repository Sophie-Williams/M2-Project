...
def __InitData():
	...
	#chrmgr.RegisterCacheEffect(chrmgr.EFFECT_SUCCESS, "",			"effect/success.mse")
	#chrmgr.RegisterCacheEffect(chrmgr.EFFECT_FAIL, "",	"effect/fail.mse")
	
	chrmgr.RegisterCacheEffect(chrmgr.EFFECT_LEVELUP_ON_14_FOR_GERMANY, "","effect/paymessage_warning.mse")	#레벨업 14일때 ( 독일전용 )
	chrmgr.RegisterCacheEffect(chrmgr.EFFECT_LEVELUP_UNDER_15_FOR_GERMANY, "", "effect/paymessage_decide.mse" )#레벨업 15일때 ( 독일전용 )
...
def __LoadGameEffect():
	...
	chrmgr.RegisterEffect(chrmgr.EFFECT_AFFECT+14, "Bip01", "d:/ymir work/pc/warrior/effect/bersek.mse") ## 천근추 (밑에도 있따-_-)
	...
	if app.ENABLE_GM_AFFECTS:
		chrmgr.RegisterEffect(chrmgr.EFFECT_AFFECT+41, "Bip01", localeInfo.FN_MOD_MARK)
		chrmgr.RegisterEffect(chrmgr.EFFECT_AFFECT+42, "Bip01", localeInfo.FN_SGM_MARK)
...
def __LoadGameNPC():
	...
	for line in lines:
		...
		try:
			if vnum:
				chrmgr.RegisterRaceName(vnum, tokens[2].strip())
				chrmgr.RegisterRaceType(tokens[2].strip(), tokens[1].strip())
			else:
				chrmgr.RegisterRaceSrcName(tokens[2].strip(), tokens[3].strip())
				chrmgr.RegisterRaceType(tokens[3].strip(), tokens[1].strip())
		except IndexError:
			import dbg
