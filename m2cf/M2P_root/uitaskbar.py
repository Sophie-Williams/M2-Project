...
class EnergyBar(ui.ScriptWindow):
	...
	def SetEnergy (self, point, leftTime, maxTime):
		...
		desc = ["","% demi-humains", "% monstre", "% critique","% perçant"," PV"," PM","% regen. pv","% abso. pv."," attaques"," attaques","","",""]
		label = "+" + str(point) + desc[constinfo.ENERGY_TYPE] + " (Restant: " + self.FormatTime(leftTime) + ")"
		abstand = ""
		...
		if point > 0:
			...
			self.tooltipEnergy.SetText("%s" % ("Aucun bonus d'énergie"))
...
class ExpandedTaskBar(ui.ScriptWindow):
	...
	def LoadWindow(self):
		...
		self.toggleButtonDict[ExpandedTaskBar.BUTTON_BONUS_GUI] = self.GetChild("BonusButton")
		self.toggleButtonDict[ExpandedTaskBar.BUTTON_BONUS_GUI].SetParent(self)
		...
		for slot in self.quickslot:
			...
			for i in xrange(4):
				...
				if player.SLOT_TYPE_INVENTORY == Type:
					...
					elif itemIndex >= 53001 and itemIndex <= 53256:
						metinSocket = [player.GetItemMetinSocket(Position, j) for j in xrange(player.METIN_SOCKET_MAX_NUM)]
