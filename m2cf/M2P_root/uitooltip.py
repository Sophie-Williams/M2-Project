...
class ItemToolTip(ToolTip):
	...
	def AppendTextLine2(self, text, color = FONT_COLOR, centerAlign = True):
	...
	# def __SetNormalItemTitle(self):
		# if chr.IsGameMaster(player.GetMainCharacterIndex()):
			# self.AppendTextLine("ID : %d" % itemVnum)
			# self.AppendSpace(5)
			# self.SetTitle(item.GetItemName())
		# else:
			# self.SetTitle(item.GetItemName())
	...
	def AddItemData(self, itemVnum, metinSlot, attrSlot = 0, flags = 0, unbindTime = 0):	
		...
		if chr.IsGameMaster(player.GetMainCharacterIndex()):
			self.AppendSpace(5)
			self.AppendTextLine2("[ Item VNum : %d ]" % itemVnum, self.NORMAL_COLOR)
	...
	if app.ENABLE_SOULBIND_SYSTEM:
		def __AppendSealInformation(self, window_type, slotIndex):
			if window_type == player.SAFEBOX:
				...
				itemSoulTime = player.GetItemBind(window_type, slotIndex)
			else:
				return
	...
	def __AppendMetinSlotInfoQuiver(self, itemVnum, metinSlot):
