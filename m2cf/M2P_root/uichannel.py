...
class ChannelChanger(ui.ScriptWindow):
	...
	def __MakeBoard(self):
		...
		self.ChangeButton.SetText(uiScriptLocale.CHSWITCH_OK)
		#self.ChangeButton.SetTop()
		...
		#self.ChannelListScrollBar.SetTop()
	...
	def __RefreshServerStateList(self):
		...
		for channelID, channelDataDict in channelDict.items():
			...
			if app.__CHANNEL_STATUS_UPDATE__:
				self.ChannelList.InsertItem(channelID, " %s %s    |cFFC7C7C7%d|r" % (channelName, channelState, channelCount))			
			else:
				self.ChannelList.InsertItem(channelID, " %s %s" % (channelName, channelState))
	...
	def __OnClickConnectButton(self):
		...
		if net.GetServerInfo().strip().split(", ")[1] == self.ChannelList.textDict[self.ChannelList.selectedLine].strip().split(" ")[0]:
			chat.AppendChat(1, localeInfo.CHSWITCH_ALREADY_USE)
			return
		elif self.__IsSpecialMap():
			chat.AppendChat(1, localeInfo.CHSWITCH_NOT_USE_MAP)
			return
