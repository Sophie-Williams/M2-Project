...
class GameWindow(ui.ScriptWindow):
	...
	def __BuildKeyDict(self):
		...
		onPressKeyDict[app.DIK_F5]	= lambda : self.QuestClickByName("M2QuestByName")
		...
		onPressKeyDict[app.DIK_U]			= self.__PressMPKey
		...
		onClickKeyDict[app.DIK_P] = lambda: self.interface.TogglePetsWindow()
	...
	def __PressMPKey(self):
		if app.IsPressed(app.DIK_LCONTROL) or app.IsPressed(app.DIK_RCONTROL):
			if 0==interfaceModule.IsWhisperHide:
	...
	# def BINARY_Acce_Abs_Succeed(self):
		# print "L'absorption de l'?ole s'est faite avec succ? !"
...
#######################  END GAYA SYSTEM ######################################
			...
			"OnResultPetAttrDetermine"		: self.__OnResultPetAttrDetermine,
			"OnResultPetAttrChange"		: self.__OnResultPetAttrChange,
	...
	def SetQuestSong(self, sound):
		x, y, z = player.GetMainCharacterPosition()
		randX = app.GetRandom(-150, 150)
		randY = app.GetRandom(-150, 150)

		snd.PlaySound3D(x+randX, -y+randY, z, "sound/quest/" +  str(sound) + ".mp3")
	...
	def SetPetEvolution(self, evo):
		petname = ["Bebe", "Sauvage", "Courageux", "Heroique"]
...
######################################## END GAYA SYSTEM	################################
	...
	#CHSwitcher = uichannel.ChannelChanger()
	#CHSwitcher.Close()
	#StateCHSwitcher = 0

	#def __switch_channel(self):
	#	if self.StateCHSwitcher == 0:
	#		self.StateCHSwitcher = 1
	#		self.CHSwitcher.Close()
	#	else:
	#		self.StateCHSwitcher = 0
	#		self.CHSwitcher.SetTop()
	#		self.CHSwitcher.Show()
