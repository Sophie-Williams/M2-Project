...
#Replace with:
	...
	def Refresh(self):
		...
		# prix_target = exchange.GetElkFromTarget()
		# prix_owner = exchange.GetElkFromSelf()

		# string_prix_target = "{:,}".format(prix_target).replace(',', '.')
		# string_prix_owner = "{:,}".format(prix_owner).replace(',', '.')

		# self.OwnerMoney.SetText(string_prix_owner)
		# self.TargetMoney.SetText(string_prix_target)

		if app.ENABLE_CHEQUE_SYSTEM:
			self.OwnerMoney.SetText(localeInfo.NumberToGoldNotTextInventory(exchange.GetElkFromSelf()))
			self.TargetMoney.SetText(localeInfo.NumberToGoldNotTextInventory(exchange.GetElkFromTarget()))
