...
class CreateCharacterWindow(ui.Window):
	...
	def Open(self):
		...
		self.gender = 0
		self.slot = -1
		self.characterface = ['warrior_m','assassin_m','sura_m','shaman_m','warrior_w','assassin_w','sura_w','shaman_w','wolfman_m']
		self.caracterjob = [localeInfo.JOB_WARRIOR, localeInfo.JOB_ASSASSIN, localeInfo.JOB_SURA, localeInfo.JOB_SHAMAN]
		self.shapeList = [
			[0, 0, 0, 0],
			[0, 0, 0, 0]]
		...
		try:
			...
			pythonScriptLoader.LoadScriptFile(dlgBoard, uiScriptLocale.LOCALE_UISCRIPT_PATH + "new_createcharacterwindow.py")

		except:
			import exception
			exception.Abort("NewCreateCharacterWindow.Open.LoadObject")
		...
		try:
			...
			self.imgface = getChild("DiscFace")
			self.textface = getChild("raceName_Text")
			...
			self.btnwarrior = getChild("WARRIOR")
			self.btnninja = getChild("ASSASSIN")
			self.btnsura = getChild("SURA")
			self.btnshamman = getChild("SHAMAN")
			self.btnwolfman = getChild("WOLFMAN")
		...
		self.btnwarrior.SetEvent(ui.__mem_func__(self.__WarriorSlotIndex))
		self.btnninja.SetEvent(ui.__mem_func__(self.__NinjaSlotIndex))
		self.btnsura.SetEvent(ui.__mem_func__(self.__SuraSlotIndex))
		self.btnshamman.SetEvent(ui.__mem_func__(self.__ShammanSlotIndex))
		self.btnwolfman.SetEvent(ui.__mem_func__(self.__WolfManSlotIndex))
	...
	def EnableWindow(self):
		...
		self.btnPrev.Enable()
		self.btnNext.Enable()
		self.btnwarrior.Enable()
		self.btnninja.Enable()
		self.btnsura.Enable()
		self.btnshamman.Enable()
		self.btnwolfman.Enable()
		#self.btnLeft.Enable()
	...
	def DisableWindow(self):
		...
		self.btnwarrior.Disable()
		self.btnninja.Disable()
		self.btnsura.Disable()
		self.btnshamman.Disable()
		self.btnwolfman.Disable()
	...
	def __SelectGender(self, gender):		
		...
		if gender == MAN:
			self.imgface.LoadImage("d:/ymir work/bin/icon/face/"+self.characterface[self.slot]+".tga")
			...
			self.imgface.LoadImage("d:/ymir work/bin/icon/face/"+self.characterface[self.slot +4]+".tga")
			for i in xrange(SLOT_COUNT):
				chr.SelectInstance(self.__GetSlotChrID(0, i))
				chr.Hide()
			for i in xrange(SLOT_COUNT):
	...
	def __SelectSlot(self, slot):
		...
		self.textface.SetText(self.caracterjob[self.slot])
	...
	def __WarriorSlotIndex(self):
		self.btnninja.SetUp()
		self.btnsura.SetUp()
		self.btnshamman.SetUp()
		self.__SelectSlot(0)
		
	def __NinjaSlotIndex(self):
		self.btnwarrior.SetUp()
		self.btnsura.SetUp()
		self.btnshamman.SetUp()
		self.__SelectSlot(1)
		
	def __SuraSlotIndex(self):
		self.btnwarrior.SetUp()
		self.btnninja.SetUp()
		self.btnshamman.SetUp()
		self.__SelectSlot(2)
		
	def __ShammanSlotIndex(self):
		self.btnwarrior.SetUp()
		self.btnninja.SetUp()
		self.btnsura.SetUp()
		self.__SelectSlot(3)
		
	def __WolfManSlotIndex(self):		
		self.PopupMessage("This character is disabled", self.EnableWindow)
		self.btnwarrior.SetUp()
		self.btnninja.SetUp()
		self.btnsura.SetUp()
		self.btnwolfman.SetUp()
