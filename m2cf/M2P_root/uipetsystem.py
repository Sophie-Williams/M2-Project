...
class PetSystemMain(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			#self.petinfo = self.GetChild("PetInfoButton")
			...
			#self.petinfo.SetEvent(ui.__mem_func__(self.OpenPetInfo))
	...
	def SetExperience(self, expm, expi, exptot):
		...
		#####
		self.tooltipexp[0].SetText(localeInfo.PET_TEXTT_2 % expmp)
		self.tooltipexp[1].SetText(localeInfo.PET_TEXTT_3 % (totexpm - expm, totexpm))
		self.tooltipexp[2].SetText(localeInfo.PET_TEXTT_4 % expip)
		self.tooltipexp[3].SetText(localeInfo.PET_TEXTT_5 % (totexpi - expi, totexpi))
	...
	def OnUpdate(self):
		if constInfo.FEEDWIND == 1 or constInfo.FEEDWIND == 3:
			if constInfo.USE_FEED != -1:
				self.feedwind.PetUseFeed(constInfo.USE_FEED)
	...
	def SetExperience(self, expm, expi, exptot):
		...
		if 0 != quarterPointi:
			if FullCounti < 1:
				...
				self.petmini_exp[4].Show()
		#####
		self.tooltipexp[0].SetText(localeInfo.PET_EXP1 % (expm, totexpm))
		self.tooltipexp[1].SetText(localeInfo.PET_EXP2 % expmp)
		self.tooltipexp[2].SetText(localeInfo.PET_EXP_ITEM1 % (expi, totexpi))
		self.tooltipexp[3].SetText(localeInfo.PET_EXP_ITEM2 % expip)
