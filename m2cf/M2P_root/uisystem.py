...
class SystemDialog(ui.ScriptWindow):
	...
	def __LoadSystemMenu_Default(self):
		...
		self.GetChild("chswitch_button").SAFE_SetEvent(self.__ClickChSwitchButton)
	...
	def Destroy(self):
		...
		if self.gameOptionCHSwitch:
			self.gameOptionCHSwitch.Destroy()
	...
	def __ClickChSwitchButton(self):
		self.Close()

		if not self.gameOptionCHSwitch:
			self.gameOptionCHSwitch = uichannel.ChannelChanger()

		self.gameOptionCHSwitch.Show()
