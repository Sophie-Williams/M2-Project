...
class SelectCharacterWindow(ui.Window):
	...
	def OnKeyDown(self, key):
		...
		if key == app.DIK_UP or key == app.DIK_LEFT:
			self.slot = (self.GetSlotIndex() - 1 + self.SLOT_COUNT) % self.SLOT_COUNT
			self.SelectSlot(self.slot)

		if key == app.DIK_DOWN or key == app.DIK_RIGHT:
			self.slot = (self.GetSlotIndex() + 1) % self.SLOT_COUNT
			self.SelectSlot(self.slot)

		if key == app.DIK_SUBTRACT or key == app.DIK_DELETE or key == app.DIK_BACK:
			self.InputPrivateCode()

		if key == app.DIK_ADD or key == app.DIK_INSERT:
			self.CreateCharacter()
