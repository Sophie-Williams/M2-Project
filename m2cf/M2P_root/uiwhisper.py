...
class WhisperDialog(ui.ScriptWindow):
	...
	def IgnoreTarget(self):
		...
		if os.path.exists("ignore.cfg"):
			...
			for i in xrange(str(ignoredList).count(",\n")):
				if str(ignoredPlayer[i]) != player:
					pass
				else:
					self.ignoreButton.Hide()
					chat.AppendChat(chat.CHAT_TYPE_INFO, "Ignore System: %s è già bloccato!" % player)
					return
		...
		self.ignoreButton.Hide()
		chat.AppendChat(chat.CHAT_TYPE_INFO, "Ignore System: utente %s bloccato!" % player)		
	...
	def SendWhisper(self):
		...
		if textLength > 0:
			...
			net.SendWhisperPacket(self.targetName, "(" + str(player.GetStatus(player.LEVEL) )+") : " + text)
			self.chatLine.SetText("")

			chat.AppendWhisper(chat.WHISPER_TYPE_CHAT, self.targetName, player.GetName() + " : (" + str(player.GetStatus(player.LEVEL)) +") : " + text)
